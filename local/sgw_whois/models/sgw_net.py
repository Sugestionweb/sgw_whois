...
f = open(get_cc_tld_path(), 'r')
# response = session.get(
#     'https://raw.githubusercontent.com/botlabio/logly/master/logly/cc_tld.txt'
#     )
cc_tld = f.read().splitlines()

f = open(get_tld_level_whois_path(), 'r')

# response = session.get(
#     'https://raw.githubusercontent.com/botlabio/logly/master/logly/tld_level_whois.txt'
#     )

tld_nic = f.read().splitlines()
...
def get_root_server(domain, server="whois.iana.org"):
    ...
    data = whois_request(domain, server, timeout=5)
...
def get_whois_raw(domain, server=None, rfc3490=True, never_cut=False, with_server_list=False, server_list=None, whois_timeout=5):
