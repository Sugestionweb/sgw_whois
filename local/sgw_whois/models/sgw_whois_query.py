...
class SgwWhoisQuery(models.Model):
    ...
    grammar["_data"]["creation_date"] = precompile_regexes(
        grammar["_data"]["creation_date"], re.IGNORECASE
    )
    grammar["_data"]["expiration_date"] = precompile_regexes(
        grammar["_data"]["expiration_date"], re.IGNORECASE
    )
    grammar["_data"]["updated_date"] = precompile_regexes(
        grammar["_data"]["updated_date"], re.IGNORECASE
    )
    grammar["_data"]["registrar"] = precompile_regexes(
        grammar["_data"]["registrar"], re.IGNORECASE
    )
    grammar["_data"]["whois_server"] = precompile_regexes(
        grammar["_data"]["whois_server"], re.IGNORECASE
    )
    grammar["_data"]["nameservers"] = precompile_regexes(
        grammar["_data"]["nameservers"], re.IGNORECASE
    )
    grammar["_data"]["emails"] = precompile_regexes(
        grammar["_data"]["emails"], re.IGNORECASE
    )
    ...
    nic_contact_references["registrant"] = precompile_regexes(
        nic_contact_references["registrant"]
    )
    ...
    nic_contact_references["billing"] = precompile_regexes(
        nic_contact_references["billing"]
    )
    ...
    def parse_raw_whois(
        ...
        try:
            data["nameservers"] = SgwWhoisQuery.remove_suffixes(data["nameservers"])
        ...
        if normalized != []:
            data = SgwWhoisQuery.normalize_data(data, normalized)
    ...
    def normalize_data(data, normalized):
        for key in ("nameservers", "emails", "whois_server"):
            if (
                ...
                and (normalized is True or key in normalized)
            ):
                if SgwWhoisQuery.is_string(data[key]):
        ...
        for key, threshold in (("registrar", 4), ("status", 3)):
            ...
            if (
                ...
                if SgwWhoisQuery.is_string(data[key]):
                    data[key] = SgwWhoisQuery.normalize_name(
                        data[key],
                        abbreviation_threshold=threshold,
                        length_threshold=1,
                        ignore_nic=ignore_nic,
                    )
                else:
                    data[key] = [
                        SgwWhoisQuery.normalize_name(
                            item,
                            abbreviation_threshold=threshold,
                            length_threshold=1,
                            ignore_nic=ignore_nic,
            ...
            if contact is not None:
                ...
                if "country" in contact and "state" in contact:
                    for country, source in (
                        ("united states", states_us),
                        ("australia", states_au),
                        ("canada", states_ca),
                    ):
                        if (
                            country in contact["country"].lower()
                            and contact["state"] in source
                        ):
                ...
                for key in ("name", "street"):
                    if (
                        ...
                        contact[key] = SgwWhoisQuery.normalize_name(
                            contact[key], abbreviation_threshold=3
                        )
                ...
                for key in ("city", "organization", "state", "country"):
                    if (
                        ...
                        contact[key] = SgwWhoisQuery.normalize_name(
                            contact[key], abbreviation_threshold=3, length_threshold=3
                        )
                ...
                if "name" in contact and "organization" not in contact:
                    ...
                    for i, line in enumerate(lines):
                        for regex in SgwWhoisQuery.organization_regexes:
    ...
    def normalize_name(
        ...
        for line in value.split("\n"):
            ...
            if (line.isupper() or line.islower()) and len(line) >= length_threshold:
                ...
                if ignore_nic is True and "nic" in line.lower():
                    ...
                    if len(words) >= 2:
                        # Last word
                        if (
                            len(words[-1]) >= abbreviation_threshold
                            and "." not in words[-1]
                        ):
    ...
    def parse_registrants(data, never_query_handles=True, handle_server=""):
        ...
        for segment in data:
            for regex in SgwWhoisQuery.registrant_regexes:
        ...
        for segment in data:
            for regex in SgwWhoisQuery.tech_contact_regexes:
                match = re.search(regex, segment)
                if match is not None:
        ...
        for segment in data:
            for regex in SgwWhoisQuery.admin_contact_regexes:
                match = re.search(regex, segment)
                if match is not None:
        ...
        for segment in data:
            for regex in SgwWhoisQuery.billing_contact_regexes:
                match = re.search(regex, segment)
                if match is not None:
        ...
        handle_contacts = SgwWhoisQuery.parse_nic_contact(data)

        # Find NIC handle references and process them
        missing_handle_contacts = []
        for category in SgwWhoisQuery.nic_contact_references:
            for regex in SgwWhoisQuery.nic_contact_references[category]:
                for segment in data:
                    match = re.search(regex, segment)
                    if match is not None:
                        data_reference = match.groupdict()
                        if (
                            data_reference["handle"] == "-"
                            or re.match(r"https?:\/\/", data_reference["handle"])
                            is not None
                        ):
                            ...
                            if found is False:
                                ...
                                if never_query_handles is False:
                                    try:
                                        contact = SgwWhoisQuery.fetch_nic_contact(
        ...
        for obj in (registrant, tech_contact, billing_contact, admin_contact):
            if obj is not None:
                for key in list(obj.keys()):
                    if (
                        obj[key] is None or obj[key].strip() == ""
                    ):  # Just chomp all surrounding whitespace
                        del obj[key]
                    else:
                ...
                if "street1" in obj:
                    ...
                    obj["street"] = "\n".join(street_items)
                if (
                    "organization1" in obj
                ):  # This is to deal with eg. HKDNR, who allow organization names in multiple languages.
                    organization_items = []
                    i = 1
                    while True:
                        try:
                ...
                if "changedate" in obj:
                    obj["changedate"] = SgwWhoisQuery.parse_dates([obj["changedate"]])[0]
                if "creationdate" in obj:
                    obj["creationdate"] = SgwWhoisQuery.parse_dates([obj["creationdate"]])[0]
                if "street" in obj and "\n" in obj["street"] and "postalcode" not in obj:
                    ...
                    if " " in lines[-1]:
                        ...
                        if (
                            "." not in lines[-1]
                            and re.match("[0-9]", postal_code)
                            and len(postal_code) >= 3
                        ):
                ...
                if (
                    ...
                    and "city" in obj
                    and (
                        re.match(r"^R\.?O\.?C\.?$", obj["country"], re.IGNORECASE)
                        or obj["country"].lower() == "republic of china"
                    )
                    ...
                    obj["country"] = "{}, {}".format(obj["city"], obj["country"])
                    lines = [x.strip() for x in obj["street"].splitlines()]
                    obj["city"] = lines[-1]
                    obj["street"] = "\n".join(lines[:-1])
    ...
    def fetch_nic_contact(handle, lookup_server):
        response = get_whois_raw(self, handle, lookup_server, whois_timeout=5)
        response = [
            segment.replace("\r", "") for segment in response
        ]  # Carriage returns are the devil
    ...
    def parse_nic_contact(data):
        handle_contacts = []
        for regex in SgwWhoisQuery.nic_contact_regexes:
            for segment in data:
    ...
    def whois(self, domain, normalized=None):
    ...
    def get_reg_whois(domain):
        temp = SgwWhoisQuery.get_whois_raw(self, domain)
