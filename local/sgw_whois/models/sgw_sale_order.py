...
class SgwSaleOrder(models.Model):
    ...
    def _cart_find_product_line(self, product_id=None, line_id=None, **kwargs):
        ...
        domain_name = ""
        if 'domain' in product_context and str.__len__(product_context['domain']) > 0 : 
            domain_name = product_context['domain'] 
    ...
    def _cart_update(self, product_id=None, line_id=None, add_qty=0, set_qty=0, **kwargs):
        ...
        if 'domain' in product_context and str.__len__(product_context['domain'] ) > 0 : 
            if not str(order_line.name).__contains__(product_context['domain']):
                order_line.write({"name":order_line.name + "\n" + product_context['domain'] })
