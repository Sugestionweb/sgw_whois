...
class CustomerPortal(CustomerPortal):
    def _prepare_portal_layout_values(self):
        ...
        values.update({"service_count": service_count})
        return values

    @http.route(
        ["/my/services", "/my/services/page/<int:page>"],
        type="http",
        auth="user",
        website=True,
    )
    def portal_my_services(
        ...
        searchbar_sortings = {
            "date": {
                "label": _("Próxima Renovación"),
                "order": "next_expiration_date asc",
            },
        ...
        if date_begin and date_end:
            domain += [
                ("next_expiration_date", ">", date_begin),
    ...
    @http.route(["/my/services/<int:id>"], type="http", auth="public", website=True)
    ...
    def portal_panel_page(self, **kw):
        ...
        return request.render("sgw_hosting.portal_panel_webmail_template", values)
