...
def parse_raw_whois(
    ...
    try:
        ...
        data["nameservers"] = remove_duplicates(
...
def normalize_data(data, normalized):
    for key in ("nameservers", "emails", "whois_server"):
        if (
            ...
            and (normalized == True or key in normalized)
        ):
...
def fetch_nic_contact(handle, lookup_server):
    response = net.get_whois_raw(handle, lookup_server, whois_timeout=5)
    response = [
